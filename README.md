...
- 다운샘플링 후 업샘플링 + Gaussian Blur 후 LQ 생성 하고, 패치별 PSNR 계산 후, 최적 k 선택하여 그리드서치
- 입력 피쳐는 아래와 같습니다. 
  - Luminance 평균 (mean)  
  - Luminance 표준편차 (std)  
  - 고주파 에너지 비율 (high_freq_ratio)  
- 총 N개 패치 데이터를 `training_dataset.csv`로 출력하여 추후에 활용
...
- `[mean, std, high_freq_ratio]`가 입력 피쳐
- 타겟 값: `target_k`  
- 학습/검증 데이터 분리 (train 80%, val 20%)  
- 평가지표: MSE (Mean Squared Error)  
- 최종 모델은 `xgb_model.joblib`으로 저장하고 추후에 추론에 사용

---

### #3 추론 및 보정 (`inference.py`)

새로운 이미지를 입력받아 패치별 k-map을 생성하고, 이를 기반으로 Adaptive Unsharp Mask를 적용합니다.  

- 원본 이미지 (`./image_input/new_img.png`)  
- 보정된 이미지 (`./result/sharpened_image.png`)  
- 출력 예시:
  [INFO] 사용된 시그마 : 2.80
  [INFO] 보정된 이미지 : (사진으로 출력)
---
...
본 프로젝트를 통해 Adaptive Unsharp Mask과 XGBoost 회귀 모델 기반의 샤프닝 시스템을 구축했습니다.  
